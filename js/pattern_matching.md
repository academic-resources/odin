...
### Patterns** There are multiple ways of potentially matching against an input. Patterns in Ruby 2.7 can be**  - Any Ruby object which is matched using `===`. The Object Pattern.
 ...
 - A Find Pattern** Patterns can also be matched using many of the patterns above together. You may for example have an array inside a hash so you could use the hash and array patterns. We'll go through each of the various patterns in more detail below :**
...
Because of this we can also check against data types** ```ruby
input = 3** case input
...
With Ruby pattern matching you can match against the following literal types**  - Booleans
 - nil
 - Numbers
 - Strings
 - Symbols
...
What happens if we have more elements in the array?** ```ruby
arr = [1, 2, 3]** case arr
in [Integer, Integer]
  puts :no_match
...
You can incorporate the variable pattern to bind matching values to variables to use later** ```ruby
