...
To see this point in action, try running the following:** ```ruby
...
Using the same example above, you can use one of pry-byebug's commands to figure out what `name = name.upcase` will return. You won't need to quit the session or add another `binding.pry` beneath it. Enter `next` to step over to the next line.   ** ```ruby
[1] pry(main)> name
=> "bob"
[2] pry(main)> greeting
=> nil
