...
**   and then run the `git push heroku main` command again.** This will send the app you created to Heroku. There will be a lot of output in your console. Wait for it to finish.** 
#### ** ** Step 3.5: Migrate the Database on Heroku** Similar to what we did locally before we launched our app, we need to migrate the database on Heroku, which we can do with the Heroku CLI.** Run this command:** ** 
```bash
